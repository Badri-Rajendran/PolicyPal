...
**Built:** comparison of real purchasable health plans from the CMS Marketplace API. That half is narrower than the Q&A — ACA individual and family plans, in the 30 states on HealthCare.gov. PolicyPal compares plans, never recommends one. All five phases in [docs/plans](docs/plans/) are complete; keeping the plan documents current is a routine, in [docs/runbooks/sbc.md](docs/runbooks/sbc.md).
