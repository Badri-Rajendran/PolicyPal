...
PolicyPal lets a logged-in user ask natural-language questions about any kind of insurance — health, auto, life, home, and more — and returns grounded answers generated by an LLM using context retrieved from a vector store.

**Built:** comparison of real purchasable health plans from the CMS Marketplace API, and for California from CMS's state-based exchange PUF (ADR 0024). That half is narrower than the Q&A — ACA individual and family plans, in the 30 states on HealthCare.gov plus California (Covered California). PolicyPal compares plans, never recommends one. All five phases in [docs/plans](docs/plans/) are complete; keeping the plan documents current is a routine, in [docs/runbooks/sbc.md](docs/runbooks/sbc.md).
...
- Refer [frontendrules](./frontend/CLAUDE.md) file for frontend development instructions.
