...
**Built:** comparison of real purchasable health plans drawn from the CMS Marketplace API, and for California from CMS's state-based exchange data (ADR 0024). That half is deliberately narrower than the Q&A — ACA individual and family plans, in the 30 states served by HealthCare.gov plus California's Covered California (190 plans from 11 issuers for 2026, priced from CMS's filed rates). The other 20 state-based exchanges, employer coverage, Medicare and Medicaid are out of scope. All five phases of [docs/plans](docs/plans/) are complete; the catalog is loaded for eighteen states (3,276 plans, 137 issuers), and 2,274 of those plans (69.4%) have their Summary of Benefits read here — 2,136 read whole and 138 in part (ADR 0017). Keeping the documents current is a routine now, not a phase: [docs/runbooks/sbc.md](docs/runbooks/sbc.md).
...
```bash
make ingest-ca-plans YEAR=2026      # ZIP=path for a file downloaded by hand, REFRESH=1 to re-download
```

The Marketplace API has no California plans, so they come from CMS's
state-based exchange PUF, one zip per plan year, published May to August of
that year (ADR 0024). The loader:

- writes the same tables, plus `rating_areas`, `plan_rates` and `catalog_loads`;
- validates the whole file before writing, in one transaction, so a bad file
  changes nothing;
- keeps a plan only in counties where it has a filed rate.

Premiums are read from `plan_rates` in SQL; CMS is never called for California.
Rating areas and issuer names are transcribed by hand and re-checked yearly:
[`docs/runbooks/california.md`](docs/runbooks/california.md). What the file
holds is in [`docs/findings/ca-sbe-puf.md`](docs/findings/ca-sbe-puf.md).

#### Source registry

Every source PolicyPal reads or links is recorded in
...
doesn't cover it. 138 of the 1,498 documents stored are partial.
...
PLAN SEARCH requires every plan question to reach `search_plans`. That includes
comparisons by metal level ("Compare the gold plans."), which used to skip it,
and two California questions that must name Covered California, never
HealthCare.gov. They need the California catalog: `make ingest-ca-plans
YEAR=2026`. CONCEPTS requires the opposite for a question about what a level
means ("What is a silver plan?"), which is answered from the corpus.
